...
class AccountRepository(ABC):
    ...
    def delete(self, account_id: UUID, user_id: UUID) -> None:
        """
        Delete an account. Does NOT commit.

        The account must belong to user_id (implementations must verify this
        before deleting, matching the ownership pattern used everywhere else).

        Raises:
            ConflictError: if the account still has transactions or transfers
                           referencing it (the database FK is ON DELETE RESTRICT
                           for exactly this reason — deleting an account with
                           financial history would silently orphan or destroy
                           that history).
        """
        ...
